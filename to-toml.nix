...
let
  inherit (lib)
    length elemAt concatMap all concatLists concatStringsSep concatMapStringsSep
    mapAttrsToList;

  inherit (builtins) abort match toJSON typeOf;

  quoteKey = k: if match "[a-zA-Z]+" k == [ ] then k else quoteString k;
  ...
  outputVal = v:
    let ty = tomlTy v;
    in if (ty == "bool" || ty == "int") then
      builtins.toJSON v
    else if ty == "string" then
      quoteString v
    else if ty == "list" || ty == "list_of_attrs" then
      let
  ...
  outputKeyVal = k: v:
    ...
    in if ty == "bool" || ty == "int" then
      ...
      abort "Not implemented: type ${ty} for key ${k}";

  tomlTy = x:
    if typeOf x == "string" then
      "string"
    else if typeOf x == "bool" then
      "bool"
    else if typeOf x == "int" then
      "int"
    else if typeOf x == "float" then
      "float"
    else if typeOf x == "set" then
      if lib.isDerivation x then "string" else "set"
    else if typeOf x == "list" then
      if length x == 0 then
        "list"
      else
        let
